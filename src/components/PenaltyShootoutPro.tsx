...
export default function PenaltyShootoutPro({ onClose }: { onClose: () => void }) {
  ...
  const takeShot = async () => {
    ...
    try {
      ...
      // NOTE: an earlier pass here switched this to MOCK_WAGER_GAMES_LONG_ZERO_ADDRESS based on
      // a comment claiming HashPack requires the long-zero form for the approve() spender. Live
      // testing showed that reverts with CONTRACT_REVERT_EXECUTED / INVALID_ALLOWANCE_SPENDER_ID —
      // the actually-deployed contract's approve spender is the CREATE2 address below.
